...
function llenarGrid(data){
  ...
  if(data.length>0){
    ...
    $.each(data,function(key, valor){
      fila += '<tr>';
      fila += '<td>'+(valor.dmisionero)+'</td>';
      fila += '<td>'+(valor.total)+'</td>';
      fila += '<td>'+((valor.claseIGL>0)?(valor.claseIGL):("-"))+'</td>';
      fila += '<td>'+((valor.claseGRP>0)?(valor.claseGRP):("-"))+'</td>';
      fila += '<td>'+((valor.claseOTR>0)?(valor.claseOTR):("-"))+'</td>';
      fila += '<td>'+((valor.condiPRP>0)?(valor.condiPRP):("-"))+'</td>';
      fila += '<td>'+((valor.condiALQ>0)?(valor.condiALQ):("-"))+'</td>';
      fila += '<td>'+((valor.condiOTR>0)?(valor.condiOTR):("-"))+'</td>';
      fila += '<td>'+((valor.regpub>0)?(valor.regpub):("-"))+'</td>';
      fila += '<td>'+((valor.escpub>0)?(valor.escpub):("-"))+'</td>';
      fila += '<td>'+((valor.contra>0)?(valor.contra):("-"))+'</td>';
      fila += '<td>'+((valor.sindocs>0)?(valor.sindocs):("-"))+'</td>';
      fila += '<td>'+((valor.resinafec>0)?(valor.resinafec):("-"))+'</td>';
      fila += '<td>'+((valor.impred>0)?(valor.impred):("-"))+'</td>';
      fila += '</tr>';
    });
