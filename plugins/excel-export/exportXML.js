var myExcelXML = (function() {
  ...
  class myExcelXML {
    constructor(o) {
      let respArray;
      let finalDataArray = [];
      try { respArray = JSON.parse(o); } 
      catch(e) { respArray = o; }

      for (let i = 0; i < respArray.length; i++) { finalDataArray.push(flatten(respArray[i])); }
        ...
        if (obj2.hasOwnProperty(k1)) {
          if (typeof obj2[k1] === 'object' && obj2[k1] !== null) {
            ...
            for (var k2 in obj2[k1]) {
              if (obj2[k1].hasOwnProperty(k2)) { obj1[k1 + '-' + k2] = obj2[k1][k2]; }
            }
          }
        }
      }
